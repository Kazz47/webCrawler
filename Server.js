...
function getIndexes(req, res, next) {
    ...
    pool.getConnection(function(err, connection) {
        if (err) {
            ...
            selectQuery = "SELECT k.Word, w.Title, u.URL FROM Keyword AS k LEFT JOIN WebpageKeywordJoin AS wkj ON wkj.KeywordId = k.Id LEFT JOIN Webpage AS w ON wkj.WebpageId = w.Id LEFT JOIN URL AS u ON u.Id = w.URLId";
